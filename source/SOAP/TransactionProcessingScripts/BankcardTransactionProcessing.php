...
if (is_array($_merchantProfileId)){
	foreach($_merchantProfileId as $profileId){
		...
		$client = new CWSClient($_identityToken, $_serviceKey, $merchProfileId['ProfileId'], $merchProfileId['ServiceId'], $_applicationProfileId,null, $_bcs);
		...
		if($_bcs->Operations->Undo)
		{
			...
			$response4 = $client->undo($response3->TransactionId, null);
